    ...
    public void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Player")
        {
			...
			if (GameObject.Find ("Player").GetComponent<Platformer2DUserControl> ().hits == 3) {
				...
				if (GameManager.instance.check == 20){
				...
				if (GameManager.instance.check == -100){
				...
				if (GameManager.instance.drops == 3) {
					...
					if (scene == 6) { //checking if going to end game screen, will need to increase index if more stages added
